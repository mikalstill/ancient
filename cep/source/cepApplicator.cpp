...
cepError
processWindow(cepDataset *ds, const cepWindow wType, string desc, int size, int overlap, string newcfname)
{
  ...
  for(int i = 0; i < cepDataset::dirUnknown; i++)
    {
      ...
      cepError werr = cepDataWindower::window(*ds->getMatrix((cepDataset::direction) i),
					      windowed[i]);
      if(werr.isReal()){
        return werr;
      }
...
cepError
processInterp(cepDataset *ds, const int iType, string desc, double sampleRate, string newcfname)
{
  ...
  for(int i = 0; i < cepDataset::dirUnknown; i++)
    {
      cepInterp myInterp;
      interped[i] = myInterp.doInterp(*ds->getMatrix((cepDataset::direction) i), 
				      sampleRate, iType);
    }
...
cepError processInterpLs(cepDataset *ds, double sampleRate, string newcfname){
  cepMatrix<double> interped[cepDataset::dirUnknown];
  
  for(int i = 0; i < cepDataset::dirUnknown; i++)
    {
      cepInterp myInterp;
      interped[i] = myInterp.LSinterp(*ds->getMatrix((cepDataset::direction) i), 
				      sampleRate, 
				      ds->getB1((cepDataset::direction) i),
  ...
  for(int i = 0; i < cepDataset::dirUnknown; i++)
    {
      ...
      cepDebugPrint("Dataset contains " + 
		    cepToString(ds->getMatrix((cepDataset::direction) i)->getNumRows()) +
		    " elements");
      ...
      for (int k = 0;; ++k)
	{
      ...
      if((ds->getMatrix((cepDataset::direction) i)->getNumRows() - fftScale) > (fftScale / 2)){
	...
	fftScale *= 2;
      ...
      cepMatrix<ComplexDble> input(fftScale,
				   ...
				   ds->getMatrix((cepDataset::direction) i)->getNumTables());
      
      // We continue the date range with padding
      float last = ds->getMatrix((cepDataset::direction) i)->
	getValue(ds->getMatrix((cepDataset::direction) i)->getNumRows() - 1, cepDataset::colDate, 0);
	...
	getValue(ds->getMatrix((cepDataset::direction) i)->getNumRows() - 2, cepDataset::colDate, 0) - 
	last;
      if(ds->getMatrix((cepDataset::direction) i)->getError().isReal()){
	return ds->getMatrix((cepDataset::direction) i)->getError();
      }
      ...
      for(int table = 0;
	  table < ds->getMatrix((cepDataset::direction) i)->getNumTables(); table++){
	for(int row = 0; row < fftScale; row++){
	  for(int col = 0; 
	      col < ds->getMatrix((cepDataset::direction) i)->getNumCols(); col++){
	    // If this entry exists
	    if(row < ds->getMatrix((cepDataset::direction) i)->getNumRows()){
	      input.setValue(row, col, table, ds->getMatrix((cepDataset::direction) i)->
      ...
      cepDebugPrint("Number of tables " + 
		    cepToString(ds->getMatrix((cepDataset::direction) i)->getNumTables()));
