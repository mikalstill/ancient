...
cepMatrix < double >
cepInterp::doInterp (cepMatrix < double >&input, double sampleRate,
		     int interpType, int winSize, double winOverlap)
{
  ...
  for (int i = 0; i < julianInput.getNumRows (); i++)
    {
      julianInput.setValue (i, 0,
			    convert.yearsToJulian (julianInput.
						   getValue (i, 0)));
    }
  ...
  numSamples =
    (int) ((julianInput.getValue (julianInput.getNumRows () - 1, 0) -
...
*/
  ...
  for (int i = 0; i < timeScale.getNumRows (); i++)
    {
      timeScale.setValue (i, 0,
			  convert.julianToYears (timeScale.getValue (i, 0)));
    }
...
cepMatrix < double >
cepInterp::doInterp (cepMatrix < double >&input,
...
cepMatrix < double >
cepInterp::nearestInterp (cepMatrix < double >&input,
			  cepMatrix < double >&timeScale)
{
  ...
  for (int i = 0; i < newSize; i++)
      ...
      if (inBounds (input, timeScale, position, i, newSize, oldSize))
	{
	  ...
	  CHECK_ERROR (input);
	  distB =
	    input.getValue (position + 1, 0) - timeScale.getValue (i, 0);
	  CHECK_ERROR (timeScale);
	  CHECK_ERROR (input);
	  if (distA < distB)
	    {
...
cepMatrix < double >
cepInterp::linearInterp (cepMatrix < double >&input,
			 cepMatrix < double >&timeScale)
{
  int newSize = timeScale.getNumRows ();
  CHECK_ERROR (timeScale);
  ...
  for (int i = 0; i < newSize; i++)
    {
      if (inBounds (input, timeScale, position, i, newSize, oldSize))
	{
	  if ((timeScale.getValue (i, 0) >
	       input.getValue (position, 0) - delta)
	    ...
	    if ((timeScale.getValue (i, 0) >
		 input.getValue (position + 1, 0) - delta)
		&& (timeScale.getValue (i, 0) <
		    input.getValue (position + 1, 0) + delta))
	    {
	      CHECK_ERROR (timeScale);
	  ...
	  else
	    {
	      ...
	      CHECK_ERROR (input);
	      distDate =
		input.getValue (position + 1, 0) - input.getValue (position,
								   0);
	      CHECK_ERROR (input);
	      distValue =
		input.getValue (position + 1, 1) - input.getValue (position,
								   1);
	      CHECK_ERROR (input);
	      timeScale.setValue (i, 1,
				  input.getValue (position,
						  1) +
				  distValue / distDate *
				  (timeScale.getValue (i, 0) -
				   input.getValue (position, 0)));
	      CHECK_ERROR (timeScale);
	      CHECK_ERROR (input);
	      timeScale.setValue (i, 3, 1.0);
	    }
...
cepMatrix < double >
cepInterp::naturalSplineInterp (cepMatrix < double >&input,
				cepMatrix < double >&timeScale)
{
  int i;
  int newSize = timeScale.getNumRows ();
  ...
  CHECK_ERROR (h);
  s.setValue (1, 0,
	      6.0 * ((input.getValue (2, 1) - input.getValue (1, 1)) /
		     h.getValue (1,
				 0) - (input.getValue (1,
						       1) - input.getValue (0,
									    1))
  ...
  for (i = 2; i < n - 1; i++)
    {
      ...
      CHECK_ERROR (h);
      t.setValue (i - 1, 1,
		  2.0 * (h.getValue (i - 1, 0) + h.getValue (i, 0)));
      CHECK_ERROR (t);
      CHECK_ERROR (h);
      ...
      CHECK_ERROR (h);
      s.setValue (i, 0,
		  6.0 * ((input.getValue (i + 1, 1) - input.getValue (i, 1)) /
			 h.getValue (i,
				     0) - (input.getValue (i,
							   1) -
					   input.getValue (i - 1,
  ...
  CHECK_ERROR (h);
  t.setValue (n - 2, 1,
	      2.0 * (h.getValue (n - 2, 0) + h.getValue (n - 1, 0)));
  CHECK_ERROR (t);
  CHECK_ERROR (h);
  s.setValue (n - 1, 0,
	      6.0 * ((input.getValue (n, 1) - input.getValue (n - 1, 1)) /
		     h.getValue (n - 1,
				 0) - (input.getValue (n - 1,
						       1) -
				       input.getValue (n - 2,
						       1)) / h.getValue (n -
									 2,
	  ...
	  if ((timeScale.getValue (i, 0) >
	  ...
	  else
	    {
	      ...
	      timeScale.setValue (i, 1,
				  a.getValue (position,
					      0) *
				  pow ((timeScale.getValue (i, 0) -
					input.getValue (position, 0)),
				       3.0) + b.getValue (position,
							  0) *
				  pow ((timeScale.getValue (i, 0) -
					input.getValue (position, 0)),
				       2.0) + c.getValue (position,
							  0) *
				  (timeScale.getValue (i, 0) -
				   input.getValue (position,
						   0)) + d.getValue (position,
								     0));
	      CHECK_ERROR (timeScale);
	      CHECK_ERROR (input);
	      CHECK_ERROR (a);
...
cepMatrix < double >
cepInterp::cubicSplineInterp (cepMatrix < double >&input,
			      cepMatrix < double >&timeScale)
{
  int i;
  int newSize = timeScale.getNumRows ();
  ...
  CHECK_ERROR (h);
  t.setValue (n - 2, 1,
	      3 * h.getValue (n - 2, 0) + 2 * h.getValue (n - 1, 0));
  CHECK_ERROR (t);
  CHECK_ERROR (h);
  s.setValue (n - 1, 0,
	      6.0 * ((input.getValue (n, 1) - input.getValue (n - 1, 1)) /
...
cepMatrix < double >
cepInterp::dividedInterp (cepMatrix < double >&input,
			  cepMatrix < double >&timeScale)
{
  int i;
  int j;
  ...
  for (j = 0; j < oldSize - 1; j++)
    {
      diffs[0]->setValue (j, 0,
			  (input.getValue (j + 1, 1) -
			   input.getValue (j, 1)) / (input.getValue (j + 1,
								     0) -
						     input.getValue (j, 0)));
      CHECK_ERROR_P (diffs[0]);
      CHECK_ERROR (input);
  ...
  for (i = 2; i < oldSize; i++)
	...
	{
	  diffs[i - 1]->setValue (j, 0,
				  (diffs[i - 2]->getValue (j + 1, 0) -
				   diffs[i - 2]->getValue (j,
							   0)) /
				  (input.getValue (j + i, 0) -
				   input.getValue (j, 0)));
	  CHECK_ERROR_P (diffs[i - 1]);
...
//        cout << "if 1\n";
		  ...
		  CHECK_ERROR (input);
		  timeScale.setValue (i, 1,
				      timeScale.getValue (i,
							  1) +
				      tempValue *
				      diffs[j]->getValue (position, 0));
...
cepMatrix < double >
cepInterp::LSinterp (cepMatrix < double >&input, double sampleRate, double m,
		     double c)
{
  ...
  int newLength =
    (int) ((julianInput.getValue (inputLength, 0) -
	    julianInput.getValue (0, 0)) / sampleRate);
  ...
  for (int i = 0; i < newLength; i++)
    {
      if (inBounds
	  (julianInput, timeScale, position, i, newLength, inputLength))
	{
	  if ((timeScale.getValue (i, 0) >
	       julianInput.getValue (position, 0) - delta)
	      && (timeScale.getValue (i, 0) <
		  julianInput.getValue (position, 0) + delta))
	    {
	      CHECK_ERROR (timeScale);
	    ...
	    if ((timeScale.getValue (i, 0) >
		 julianInput.getValue (position + 1, 0) - delta)
		&& (timeScale.getValue (i, 0) <
		    julianInput.getValue (position + 1, 0) + delta))
	    {
	      CHECK_ERROR (timeScale);
	      CHECK_ERROR (julianInput);
	      timeScale.setValue (i, 1,
				  julianInput.getValue (position + 1, 1));
	      CHECK_ERROR (timeScale);
	      CHECK_ERROR (julianInput);
	      timeScale.setValue (i, 3, 0.0);
	      CHECK_ERROR (timeScale);
	  ...
	  else
	    {
	      ...
	      CHECK_ERROR (julianInput);
	      timeScale.setValue (i, 1,
				  m * (timeScale.getValue (i, 0) -
				       julianInput.getValue (0, 0)) + c);
	      CHECK_ERROR (timeScale);
	      CHECK_ERROR (julianInput);
	      timeScale.setValue (i, 3, 1.0);
	    }
...
bool
  cepInterp::inBounds (cepMatrix < double >&input,
		       cepMatrix < double >&timeScale, int &position, int &i,
		       int newSize, int oldSize)
{

  if ((position < oldSize - 1)
      && input.getValue (position + 1, 0) < timeScale.getValue (i, 0) + delta)
    {
...
void
cepInterp::rowReduce (cepMatrix < double >&t, cepMatrix < double >&s, int n)
// Imports:
  ...
  for (i = 1; i < n - 1; i++)
    {
      ...
      t.setValue (i, 1,
		  t.getValue (i, 1) - t.getValue (i - 1, 2) * t.getValue (i,
									  0));
      s.setValue (i + 1, 0,
		  s.getValue (i + 1, 0) - s.getValue (i, 0) * t.getValue (i,
									  0));
      t.setValue (i, 0, 0.0);
    }
  // for last row
...
void
cepInterp::calc_abc (cepMatrix < double >&a, cepMatrix < double >&b,
		     cepMatrix < double >&c, cepMatrix < double >&s,
		     cepMatrix < double >&h, cepMatrix < double >&input,
		     int n)
{
  int i;
  ...
  for (i = 0; i < n; i++)
    {
      a.setValue (i, 0,
		  (s.getValue (i + 1, 0) -
		   s.getValue (i, 0)) / (6.0 * h.getValue (i, 0)));
      b.setValue (i, 0, s.getValue (i, 0) * 0.5);
      c.setValue (i, 0,
		  (input.getValue (i + 1, 1) -
		   input.getValue (i, 1)) / h.getValue (i,
							0) - (h.getValue (i,
									  0) *
							      (2.0 *
							       s.getValue (i,
...
void
cepInterp::setColour (cepMatrix < double >&input,
		      cepMatrix < double >&timeScale, int position, int i)
{
  double delta = 0.0001;
  if (((timeScale.getValue (i, 0) < input.getValue (position, 0) + delta) &&
