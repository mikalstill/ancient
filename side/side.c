...
int main(int argc, char *argv[]){
  char temp[2][100], fmt[255], cmd[2][1024], *file[2], color[2][10], line[LINEWIDTH];
  int fd[2], cnt[2], done;
  struct stat sb[2];
  off_t inset[2];
  ...
  // Setup the color control codes. This should be integrated with getopt one day...
  snprintf(color[0], 10, "%c[34m", 27); // blue
  snprintf(color[1], 10, "%c[31m", 27); // red

  // Filter the files into temporary locations -- open to format string attacks, as
  // we as temporary file substitution attacks
  ...
  for(currfile = 0; currfile < 2; currfile++){
    ...
    if ((file[currfile] =
	 (char *) mmap (NULL, sb[currfile].st_size, PROT_READ, MAP_SHARED, fd[currfile], 0)) == -1)
      {
  ...
  while((inset[0] < sb[0].st_size) || (inset[1] < sb[1].st_size)){
	...
	while((count[currfile]++ < (LINEWIDTH / 2) - 1) && (file[currfile][inset[currfile]] != '\n')){
	  line[count[currfile] + (currfile * LINEWIDTH / 2) + currfile] = file[currfile][inset[currfile]++];
	  if(file[currfile][inset[currfile] - 1] == '\t') count[currfile] += 7;
	}      
      }
    ...
    if(strncmp(line, line + (LINEWIDTH / 2) + 1, (LINEWIDTH / 2) - 2) == 0)
      printf("%s  ", color[0]);
    else printf("%s++", color[1]);
    printf("%s", line);
  }
