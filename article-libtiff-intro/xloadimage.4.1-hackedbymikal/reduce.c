...
{ unsigned long pixel_counts[32768]; /* pixel occurrance histogram */
  ...
  case IRGB:
    for (y= 0; y < image->height; y++)
      for (x= 0; x < image->width; x++) {
	/* we use pixval as a temporary because some compilers have
	 * trouble with the complexity of the inlined statements otherwise.
	 */
	pixval = pixel_counts[TLA_TO_15BIT(image->rgb,
					   memToVal(pixel, image->pixlen))];
	valToMem(pixval, dpixel, new_image->pixlen);
	pixel += image->pixlen;
  ...
  case ITRUE:
    for (y= 0; y < image->height; y++)
      for (x= 0; x < image->width; x++) {
	pixval = pixel_counts[TRUE_TO_15BIT(memToVal(pixel, 3))];
	valToMem(pixval, dpixel, new_image->pixlen);
	pixel += 3;
	dpixel += new_image->pixlen;
      }
...
{ int x, y, linelen;
  ...
  /* if it's not a one-bit image then forget it
   */
  //  if (!RGBP(image) || (image->rgb.used > 2)){
  //    printf("Mikal: Bail out because image->rgb.used = %d\n",
  //	   image->rgb.used);
  //    return(image);
  //  }
    
  new = newBitImage(image->width * 8, image->height);
  new->title = dupString(image->title);

  /* copy colormap
   */
  if (image->rgb.used > 0) {
    new->rgb.red[0] = image->rgb.red[0];
    new->rgb.green[0] = image->rgb.green[0];
    new->rgb.blue[0] = image->rgb.blue[0];
  }
  if (image->rgb.used > 1) {
    new->rgb.red[1] = image->rgb.red[1];
    new->rgb.green[1] = image->rgb.green[1];
    new->rgb.blue[1] = image->rgb.blue[1];
  }
  ...
  sp = image->data;
  last_dp = new->data;
  linelen = (new->width / 8) + (new->width % 8 ? 1 : 0);
  for (y = 0; y < image->height; y++) {
    dp = last_dp;
    mask = 0x80;
    for (x = 0; x < image->width; x++) {
      if (memToVal(sp, image->pixlen) != 0)
	*dp |= mask;
      sp += image->pixlen;
      mask >>= 1;
      if (mask == 0) {
